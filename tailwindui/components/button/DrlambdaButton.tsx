...
export const BigBlueButton: React.FC<DrlambdaButtonProps> = ({ children, onClick, isSubmitting = false, isPaidUser, isPaidFeature = false }) => {
  return (
    <button className='btn w-[100px] sm:w-[120px] h-[36px] sm:h-[36px] sm:gap-x-2 text-white sm:font-semibold bg-blue-600 disabled:bg-gray-600 disabled:animate-pulse whitespace-nowrap rounded-xl' onClick={onClick} disabled={isSubmitting}>
      {children}
    </button>
  )
}
