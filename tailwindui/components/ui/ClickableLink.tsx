...
const ClickableLink: React.FC<ClickableLinkProps> = ({ link }) => {
    ...
    return (
        <div
            className="h-fit w-full form-input flex flex-row flex-nowrap border border-gray-500 p-0 cursor-text rounded-xl mb-6"
            ...
            <input
                id="search_keyword"
                type="text"
                className="flex-grow border-0 p-0 h-6 focus:outline-none focus:ring-0 mx-3 my-3 w-full overflow-hidden cursor-text text-[#707C8A]"
                readOnly
                value={link}
            />
        </div>
