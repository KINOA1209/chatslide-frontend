...
const LinkInput = ({
	...
	async function addLink(link: string) {
		...
		if (link.startsWith('file:///')) {
			setLinkError(
				'This is a file link, please use the window above to upload.',
			);
			return;
		}
	...
	return (
		<div className='bg-gray-100 border border-2 rounded-lg border-gray-200'>
				...
				<div className='w-full'>
					...
					<input
						...
						onKeyDown={(e) => {
							if (e.key === 'Enter') {
								addLink(linkUrl);
								e.preventDefault(); // Optionally prevent the default behavior
							}
						}}
			...
			{linkError && (
				<div className='text-sm text-red-500 mb-3 ml-3'>{linkError}</div>
			)}
		</div>
	);
};
