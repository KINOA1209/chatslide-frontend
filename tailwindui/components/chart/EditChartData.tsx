...
const EditChartData: React.FC<EditChartDataProps> = ({ 
    ...
    useEffect(() => {
      const calculateMaxHeight = () => {  
        if (imgModuleRef.current && topButtonRef.current && titleRef.current && typeRef.current && doneButtonRef.current) {
    ...
    const handleAddDataPoint = () => {
        if (chartSelectionConfigs[chartType].dataStructure === 'ScatterDataPoint') {
          ...
          const newDataPoint = { label: '', value: 0, color: colorPalette[chartData.length % colorPalette.length]} as ValueDataPoint;
            ...
            <>
              ...
              <div className="relative" ref={pickerRef}>
                <div 
                  ...
                  onClick={() => setActiveColorPicker(index === activeColorPicker ? null : index)}
                />
                {activeColorPicker === index && (
                  <div className="absolute left-full top-0 ml-2 z-50">
                    <ChromePicker
                      color={dataPoint.color}
                      onChange={(color) => {handleDataChange(index, 'color', color.hex);}}
                    />
                  </div>
                )}
              </div>
    ...
    return (
      <div>
        ...
        <div
          ...
          style={{
            maxHeight: maxContentHeight !== null ? `${maxContentHeight}px` : 'none',
            minHeight: maxContentHeight !== null ? `${maxContentHeight}px` : 'none',
          }}
          ref={renderDataRef}
        >
          {renderDataFields()}
