...
import {
	FromCloudIcon,
	FromComputerIcon,
} from '@/app/(feature)/my-resources/icons';
...
const UploadToLibraryWindow: React.FC<UploadToLibraryWindowProps> = ({
	...
	const [selectedUploadedResources, setSelectedUploadedResources] = useState<
		Array<File | null>
	>([]);
	const [uploadedResourcesClicked, setUploadedResourcesClicked] = useState(
		new Array(uploadedResources.length).fill(false),
	);
	...
	return (
		<section>
			<div className='fixed top-0 left-0 w-full h-full flex items-center justify-center z-[20]'>
				...
				<div
					id='main_container'
					className='w-full md:w-[34rem] h-[35rem] relative rounded-lg flex flex-col items-center'
				>
					...
					<div className='flex flex-row w-full px-5'>
						{/* From computer tab */}
						<div
							className={`flex-1 h-10 px-3 py-1 ${
								activeTab === 'computer'
									? 'border-b-2'
									: 'border-b-0 opacity-50'
							} bg-neutral-800 bg-opacity-0 justify-center items-center gap-2.5 inline-flex border-black cursor-pointer`}
							onClick={(e) => {
								e.stopPropagation();
						...
						>
							...
							<div
								className={`text-center text-neutral-900 text-sm font-medium font-creato-medium leading-normal tracking-wide`}
							>
								From Computer
							</div>
						</div>
						...
						<div
							className={`flex-1 h-10 px-3 py-1 ${
								activeTab === 'cloud' ? 'border-b-2' : 'border-b-0 opacity-50'
							} bg-neutral-800 bg-opacity-0 justify-center items-center gap-2.5 inline-flex border-black cursor-pointer`}
							onClick={(e) => {
								e.stopPropagation();
					...
					{activeTab === 'computer' && (
						<div className='h-[400px] my-[20px] px-2 md:px-5 w-full flex-1 flex flex-col'>
							<div className='flex-1 overflow-auto max-h-[280px] z-[20]'>
								{uploadedResources.map((uploadedFile, index) => (
									<li className='list-none flex' key={index}>
										<div
											id='uploadfile_each'
											className='flex w-full justify-between items-center gap-2 rounded h-[45px] pl-[1rem] mt-[10px]'
										>
											...
											<button
												className={`uploadButton ${
													uploadedResourcesClicked[index] ? 'clicked' : ''
												} flex-end w-[17px] h-[17px] mr-[20px] rounded-full hover:bg-gray-400 transition duration-300`}
							...
							<div
								...
								className={`w-full min-h-[100px] border-0 border-dotted border-gray-400'} bg-gray-200 rounded-lg flex flex-col items-center justify-center`}
							>
								{' '}
                  ...
                  or drop here
					...
					{activeTab === 'cloud' && (
						...
						>
							<div className='h-[385px] my-[20px] flex items-center justify-center'>
								<button
									className='w-full btn text-white font-bold bg-black from-blue-600 to-teal-500 z-[50]'
									type='button'
								>
					...
					<div className='flex flex-row w-full items-center h-[50px] justify-between px-5'>
						...
						<div className='w-[80.81px] h-7 bg-zinc-600 rounded-[4.94px] justify-center items-center inline-flex'>
							<div
								className='text-center text-zinc-100 text-sm font-medium font-creato-medium leading-[14.81px] tracking-wide cursor-pointer'
								onClick={() => {
